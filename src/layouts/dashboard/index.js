...
function Dashboard() {
    ...
    fetch(
      ...
      .then((result) => {
        ...
        if (isMounted) {
          ...
          const firstDays = new Date(date.getFullYear(), date.getMonth(), 1);
          const lastDayx = new Date(date.getFullYear(), date.getMonth(), date.getDate());
  ...
  useEffect(() => {
    ...
    // const orgIDs = data11.orgID;
    let isMounted = true;
    fetch(`${process.env.REACT_APP_EKOATLANTIC_URL}/audit/getFilter`, { headers })
      .then(async (res) => {
        const aToken = res.headers.get("token-1");
        localStorage.setItem("rexxdex", aToken);
        return res.json();
